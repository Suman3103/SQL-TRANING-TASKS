...
-- task 1.3
-- 1 find all employees hired after 2000
	select 	* from employee           -- used select clause to select details from employee table and where function to filter
    where hiredate > '1999-12-31';
-- 2 find employees with salary between 40,000 and 80,000 
    SELECT NAME, SALARY                 -- USED BETWEEN CLAUSE TO GET RANGE BETWEEN 2 VALUES
    FROM EMPLOYEE
    WHERE SALARY BETWEEN 40000 AND 80000;
-- 3FIND EMPLOYEES WHOSE NAME STARTS WITH 'A'
	SELECT NAME 				-- USED LIKE CLAUSE TO GET MATCHING STRING
    FROM EMPLOYEE 
    WHERE NAME LIKE 'A%';
-- 4 LIST ALL THE DEPT (NO DUPLICATE)
    SELECT DISTINCT DEPARTMENT    -- USED DISTINCT KEY WORD TO REMOVE DUPLICATE
    FROM EMPLOYEE;
-- 5 SORT EMPLOYEES BY SALARY DESC AND THE BY NAME ASC
select NAME, SALARY 
FROM EMPLOYEE            -- ORDER BY IS USE TO SORT 
